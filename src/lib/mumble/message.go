...
func (server *Server) handleChannelStateMessage(client *Client, message *Message) {
	...
	if channelState.Description != nil {
		...
		if err != nil {
			client.sendPermissionDeniedType(protocol.PermissionDenied_TextTooLong)
			return
		}
	}
		...
		if channel != nil && channel.ID != 0 {
			...
			for _, childChannel := range channelParent.children {
				...
				if childChannel.Name == name {
					client.sendPermissionDeniedType(protocol.PermissionDenied_ChannelName)
					return
				}
			}
		}
	...
	if channel == nil {
		...
		if !client.IsRegistered() && !client.HasCertificate() {
			client.sendPermissionDeniedTypeUser(protocol.PermissionDenied_MissingCertificate, client)
			return
		}

		// We can't add channels to a temporary channel
		if parent.IsTemporary() {
			client.sendPermissionDeniedType(protocol.PermissionDenied_TemporaryChannel)
			return
		}
...
func (server *Server) handleUserStateMessage(client *Client, message *Message) {
	...
	if userState.ChannelID != nil {
		...
		if maxUsersPerChannel != 0 && destinationChannel.clients[maxUsersPerChannel] != nil {
			client.sendPermissionDeniedFallback(protocol.PermissionDenied_ChannelFull, 0x010201, "Channel is full")
			return
		}
	}
	...
	if userState.Mute != nil || userState.Deaf != nil || userState.Suppress != nil || userState.PrioritySpeaker != nil {
		...
		if target.IsSuperUser() {
			client.sendPermissionDeniedType(protocol.PermissionDenied_SuperUser)
			return
		}
	...
	if userState.UserID != nil {
		...
		if !target.HasCertificate() {
			client.sendPermissionDeniedTypeUser(protocol.PermissionDenied_MissingCertificate, target)
			return
		}
	}
...
func (server *Server) handleAclMessage(client *Client, message *Message) {
	...
	//	// in our channel list.
	//	reply.ACLs = []*protocol.ACL_ChanACL{}
	...
	//	reply.Groups = []*protocol.ACL_ChanGroup{}
	...
	//	queryUsers := &protocol.QueryUsers{}
