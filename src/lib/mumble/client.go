...
func (client *Client) RejectAuth(rejectType protocol.Reject_RejectType, reason string) {
...
func (c *Client) sendPermissionDeniedType(denyType protocol.PermissionDenied_DenyType) {
	c.sendPermissionDeniedTypeUser(denyType, nil)
}

// Send permission denied by type (and user)
func (client *Client) sendPermissionDeniedTypeUser(denyType protocol.PermissionDenied_DenyType, user *Client) {
...
func (client *Client) sendPermissionDeniedFallback(denyType protocol.PermissionDenied_DenyType, version uint32, text string) {
	permissionDenied := &protocol.PermissionDenied{
		Type: denyType.Enum(),
	}
...
func (client *Client) udpReceiveLoop() {
	for buffer := range client.udpReceive {
		...
		switch kind {
		case protocol.UDPMessageVoiceSpeex:
			fallthrough
		case protocol.UDPMessageVoiceCELTAlpha:
			fallthrough
		case protocol.UDPMessageVoiceCELTBeta:
			if client.server.Opus {
				return
			}
			fallthrough
		case protocol.UDPMessageVoiceOpus:
			...
			if kind != protocol.UDPMessageVoiceOpus {
				for {
...
func (client *Client) sendMessage(message interface{}) error {
	...
	kind = protocol.MessageType(message)
...
func (client *Client) tlsReceiveLoop() {
	for {
		...
		if client.state == StateClientReady {
			...
			if message.kind == protocol.MessageUDPTunnel {
				client.udp = false
